...
function App() {
  ...
  const requestSignUp = (formSignUp) => {
    axios.post('https://server-medical-blog.vercel.app/signup', formSignUp)
        .then(response => {
  ...
  useEffect(() => {
    ...
    if (loggedInStatus === 'true') {
      ...
      axios.get(`https://server-medical-blog.vercel.app/notification/${localStorage.getItem('idUser')}`)
      .then(response => {
